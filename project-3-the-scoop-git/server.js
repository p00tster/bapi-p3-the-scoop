...
const fs = require('fs');
const yamlDatabase = 'database.yml';
...
//CodeAcademy assignment code begins here
...
//YAML code for local DB
function loadDatabase() {
  let openDatabase;
  if(fs.accessSync(yamlDatabase)) {
    console.log(`ERROR: The file ${yamlDatabase} does not exist in main directory.`);
  } else {
    openDatabase = yaml.safeLoad(fs.readFileSync(yamlDatabase,'utf8'));
  }
  return openDatabase;
}

function saveDatabase() {
    fs.writeFileSync(yamlDatabase, yaml.safeDump(database));
}
